...
bool D3DClass::Initialize(int screenWidth, int screenHeight, bool vsync, HWND hwnd, bool fullscreen, float screenDepth, float screenNear)
{
	...
	// 팩토리 객체를 사용하여 첫번째 그래픽 카드 인터페이스 어댑터를 생성
	...
	// 출력 (모니터)에 대한 DXGI_FORMAT_R8G8B8A8_UNORM 표시 형식에 맞는 모드 수를 가저옵니다.
	...
	for (unsigned int i = 0; i < numModes; i++)
	{
		if (displayModeList[i].Width == (unsigned int)screenWidth)
		{
			...
			{
				numberator = displayModeList[i].RefreshRate.Numerator;
	...
	if (m_vsync_enabled)
	{
		swapChainDesc.BufferDesc.RefreshRate.Numerator = numberator;
	...
	// 스왑체인, Direct3D 장치 및 Direct3D 장치 컨텐스트를 만듭니다.
	...
	// 그려지는 폴리곤과 방법을 결정할 레스터 구조체를 설정합니다.
...
{
	// 종료 전 윈도우 모드로 설정하지 않으면 스왑 체인을 해제할 때 예외가 발생합니다.
	if (m_swapChain)
	{
